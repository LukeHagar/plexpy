...
class Plex:
    ...
    def get_home_data(self) -> operations.GetHomeDataResponse:
        r"""Get Plex Home Data
        Retrieves the home data for the authenticated user, including details like home ID, name, guest access information, and subscription status.
        """
        hook_ctx = HookContext(operation_id='getHomeData', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        res = operations.GetHomeDataResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type') or '', raw_response=http_res)
        
        if http_res.status_code == 200:
            # pylint: disable=no-else-return
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[operations.GetHomeDataResponseBody])
                res.object = out
            else:
            ...
            # pylint: disable=no-else-return
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'application/json'):                
                out = utils.unmarshal_json(http_res.text, errors.GetHomeDataResponseBody)
    ...
    def get_pin(self, x_plex_product: str, strong: Optional[bool] = None, x_plex_client_identifier: Optional[str] = None, server_url: Optional[str] = None) -> operations.GetPinResponse:
        r"""Get a Pin
