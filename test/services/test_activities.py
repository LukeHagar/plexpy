...
class TestActivities_(unittest.TestCase):
    ...
    def test_cancel_server_activities(self):
        # Mock the API response
        responses.delete(
            "http://10.10.10.47:32400/activities/4619061582", json={}, status=200
        )
        # call the method to test
        test_service = Activities("testkey")
        response = test_service.cancel_server_activities("4619061582")
        self.assertEqual(response.data, {})
        responses.reset(),
    ...
    def test_cancel_server_activities_required_fields_missing(self):
        # Mock the API response
        responses.delete(
            "http://10.10.10.47:32400/activities/7372757977", json={}, status=202
        )
    ...
    def test_cancel_server_activities_error_on_non_200(self):
        # Mock the API response
        responses.delete(
            "http://10.10.10.47:32400/activities/8133937440", json={}, status=404
        )
        with self.assertRaises(ClientException):
            test_service = Activities("testkey")
            test_service.cancel_server_activities("8133937440")
        responses.reset()
